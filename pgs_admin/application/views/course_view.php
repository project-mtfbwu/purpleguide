                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body table-responsive">
                                        <h4 class="m-t-0 header-title"><b>View Courses</b></h4>
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            <h4 class="card-title m-0"></h4>
                                            <a href="<?= base_url('Courses/add_course') ?>" class="btn btn-primary">Add Course</a>
                                        </div>
                                        ...
                                        <?php if ($row->block_status == 0) { ?>
                                                    <a class="btn btn-primary btn-sm" href="<?= base_url('Courses/block/' . $row->id); ?>">
                                                        Published
                                                    </a>
                                                <?php } else { ?>
                                                    <a class="btn btn-danger btn-sm" href="<?= base_url('Courses/block/' . $row->id); ?>">
                                                        Unpublished
                                                    </a>
                                        <?php } ?>
                                                        </td>
                                                <td style="text-align:center;">
                                                    <input type="checkbox" class="picks-toggle" data-id="<?= $row->id ?>"
                                                        <?= !empty($row->show_in_picks) ? 'checked' : '' ?>
                                                        style="width:18px;height:18px;cursor:pointer;" title="Show in #Pgs Picks">
                                                </td>
                                                 <td>
                                                            <a class="btn btn-outline-success btn-sm edit" href="<?= base_url('Courses/edit_course/').$row->id ?>" title="Edit">
...
function confirmDelete(id)
 {
    if (confirm("Are you sure you want to delete this data?")) {
        window.location.href = "<?php echo base_url()?>Courses/delete_course/" + id;
    }
}
document.addEventListener('change', function (e) {
    if (!e.target.matches('.picks-toggle')) return;
    var cb = e.target;
    var id = cb.dataset.id;
    fetch('<?= base_url('Courses/toggle_picks/') ?>' + id)
        .then(function (r) { return r.json(); })
        .then(function (data) { if (!data.success) cb.checked = !cb.checked; })
