...
class Testimonial extends CI_Controller {
   public function __construct(){
    ...
    {
        $data['product'] = $this->db->order_by('id', 'desc')->get('testimonial_tbl')->result();
    ...
    public function add_testimonial_data()
    {
        ...
        $id = $this->um->insert('testimonial_tbl', $prod_data);
    ...
    public function edit_testimonial($id)
    {
        ...
        $article = $this->db->get_where('testimonial_tbl', ['id' => $id])->row();
    ...
    public function edit_testimonial_data()
    {
        ...
        if (empty($name)) {
            ...
            redirect(base_url('Testimonial/edit_testimonial/') . $p_id);
            return;
        }
        ...
        $updated = $this->um->update('testimonial_tbl', ['id' => $p_id], $prod_data);
   ...
   public function block($id)
    {
    $id = $this->uri->segment(3);
    $sql = $this->um->get_dataa('testimonial_tbl',['id'=>$id]);
   ...
   if ($status==0) {
      ...
      $data= $this->um->update('testimonial_tbl',['id'=>$id],['block_status'=>1]);
       $this->session->set_flashdata('success', 'Testimonial Unpublished Successfully');
       redirect (base_url().'Testimonial');die();

   }else{
      
         $data= $this->um->update('testimonial_tbl',['id'=>$id],['block_status'=>0]);
          $this->session->set_flashdata('success', 'Testimonial Published Successfully');
           redirect (base_url().'Testimonial');die();
        }
    }else{
         
          $this->session->set_flashdata('error', 'Testimonial id not found');
           redirect (base_url().'Testimonial');die();
    }
   }
   public function delete_testimonial(){
      $id = $this->uri->segment(3);
         $res = $this->um->delete('testimonial_tbl',['id'=>$id]);
