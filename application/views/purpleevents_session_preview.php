...
// Use uploaded preview image (website tmp) if present, else fall back to normal event image helper.
$img_src = '';
if (!empty($event->preview_image) && is_string($event->preview_image)) {
    $img_src = base_url(ltrim($event->preview_image, '/'));
} else {
...
<body data-mobile-nav-style="classic" class="custom-cursor">
    ...
    <div style="position:sticky;top:0;z-index:99999;background:#fff3cd;border-bottom:1px solid #ffeeba;padding:10px 12px;font-size:14px;color:#856404;">
        <b>Preview mode:</b> this does not save the event.
        <span style="margin-left:10px;">Close this tab to continue editing.</span>
    </div>

<?php
// The rest of this file is intentionally the same as purpleevents_session.php,
// except facilitator image rendering (supports tmp preview images).
?>
    ...
    <header>
                ...
                <div class="col-lg-8 align-items-center">
                    <div class="me-lg-0 me-auto">
                        <a class="navbar-brand" href="<?= base_url() ?>">
                            <img src="<?= base_url('assets/img/logo.png')?>" alt="" class="default-logo">
                            <img src="<?= base_url('assets/img/logo.png')?>" alt="" class="alt-logo">
                            <img src="<?= base_url('assets/img/logo.png')?>" alt="" class="mobile-logo">
                        </a>
                    </div>
                    <div class="col-auto menu-order position-static">
                        <ul class="header-tab-top">
                            <li><a href="<?= base_url('Simplehome')?>" class="tab-button text-black" style="padding: 4px;">#feed</a></li>
                            <li><a href="<?= base_url('purplepremiumhome')?>" class="tab-button active-tab text-black" style="padding: 4px;">#purplePremium</a></li>
                            <li><button class="tab-button" type="button">#cvReadyPrograms</button></li>
                        </ul>
                    </div>
                </div>
        ...
        <div class="mobile-block mobile-header">
            <div class="d-flex d-flex-space justify-content-space">
                <div><a href="<?= base_url('/')?>"><img src="<?= base_url('assets/img/white-logo.png')?>" alt=""></a></div>
                <div class="d-flex d-flex-space justify-content-space">
                    <button type="button" class="btn-toggle-mobile"><img src="<?= base_url('assets/img/notification.png')?>" alt=""></button>
                    <?php if($this->session->userdata('logged_in')): ?>
                        <a href="<?= base_url('Login/logout')?>" class="btn btn-login">Logout</a>
                    <?php else: ?>
                        <a href="<?= base_url('Login')?>" class="btn btn-login">Login</a>
                    <?php endif; ?>
                    <button type="button" class="btn-toggle-mobile"><img src="<?= base_url('assets/img/toggle-lines.png')?>" alt=""></button>
                </div>
            </div>
        </div>
    </header>
                ...
                <div class="row justify-content-center align-items-center gap-5">
                    <div class="w-650px p-0 mobile-w-full">
                        ...
                        <div class="d-flex gap-3 justify-content-center flex-nowrap" style="flex-wrap: nowrap;">
                            ...
                            <?php
                                $facImg = '';
                                if (!empty($fac->image) && is_string($fac->image) && strpos($fac->image, 'assets/tmp/') === 0) {
                                    $facImg = base_url($fac->image);
                                } else {
                                    $facImg = htmlspecialchars(facilitator_image_url($fac->image ?? null, base_url('assets/img/founder.png')));
                                }
                            ?>
                            <div class="w-50 mobile-w-50 mobile-mt-0 mobile-pt-0">
                                <div class="founder-img-box border-radius-4px mb-2 w-full border-radius-20px">
                                    <img src="<?= $facImg ?>" alt="<?= htmlspecialchars($fac->name ?? '') ?>" data-no-retina="" onerror="this.src='<?= base_url('assets/img/founder.png') ?>'">
                                </div>
