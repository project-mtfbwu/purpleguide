...
Class Home extends CI_Controller{
    ...
    public function submit_study_journey(){
        ...
        $this->form_validation->set_rules('stream', 'Stream', 'required|trim|callback_valid_study_journey_stream');
        $this->form_validation->set_rules('country', 'Current Journey Step', 'required|trim|callback_valid_study_journey_choice[country]');
        $this->form_validation->set_rules('study_level', 'Level of Study', 'required|trim|callback_valid_study_journey_study_level');
        ...
        // The form now sends a single "group|value" pick for the stream and for
        // the level of study. Validation above guarantees both parse cleanly.
        $stream = $this->_parse_study_journey_combo(
            $this->input->post('stream', true),
            $this->_stream_group_map()
        );
        $study_level = $this->_parse_study_journey_combo(
            $this->input->post('study_level', true),
            $this->_study_level_group_map()
        );
        ...
        $data = [
            ...
            'current_medical_path' => '',
            'current_masters_path' => '',
            'current_undergrad_path' => '',
        ...
        // Store the single chosen option in its matching column.
        $data[$stream['column']] = $stream['value'];
        $data[$study_level['column']] = $study_level['value'];
    ...
    // Maps each "Pick your stream" group to the DB column it is stored in and
    // its human-readable label (shown in admin).
    private function _stream_group_map(){
        return [
            'medical1'  => ['column' => 'medical_path',   'label' => 'Medical Path'],
            'masters'   => ['column' => 'masters_path',   'label' => 'Masters Path'],
            'undergrad' => ['column' => 'undergrad_path', 'label' => 'Undergrad Path'],
            'medical2'  => ['column' => 'medical_path_2', 'label' => 'Medical Path 2'],
        ];
    }

    // Maps each "Level of your study" group to its DB column and label.
    private function _study_level_group_map(){
        return [
            'medicalpath'   => ['column' => 'current_medical_path',   'label' => 'Medical Path'],
            'masterpath'    => ['column' => 'current_masters_path',   'label' => 'Masters Path'],
            'undergradpath' => ['column' => 'current_undergrad_path', 'label' => 'Undergrad Path'],
        ];
    }

    // Splits a "group|value" radio submission (e.g. "medical1|USMLE"), validates
    // the group and value, and returns the target column/label. Null if invalid.
    private function _parse_study_journey_combo($raw, $group_map){
        $raw = (string) $raw;
        $pos = strpos($raw, '|');
        if ($pos === false) {
            return null;
        ...
        $options = $this->_study_journey_options();
        if (!isset($options[$group]) || !in_array($value, $options[$group], true)) {
            return null;
        }
        ...
        return [
            'group'  => $group,
            'value'  => $value,
            'column' => $group_map[$group]['column'],
            'label'  => $group_map[$group]['label'],
        ];
    }

    public function valid_study_journey_stream($value){
        if ($this->_parse_study_journey_combo($value, $this->_stream_group_map()) === null) {
            $this->form_validation->set_message('valid_study_journey_stream', 'Please pick your stream.');
            return false;
        }
        return true;
    }

    public function valid_study_journey_study_level($value){
        if ($this->_parse_study_journey_combo($value, $this->_study_level_group_map()) === null) {
            $this->form_validation->set_message('valid_study_journey_study_level', 'Please select your level of study.');
            return false;
        }
        return true;
    }
